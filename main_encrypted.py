...
#------------------------------------------------------------
# AES-128 UDP ENCRYPTION CONFIGURATION
#------------------------------------------------------------

UDP_ENCRYPTION_ENABLED = True
AES_PASSPHRASE = b"teamsocket"
AES_BLOCK_SIZE = 16
...
#------------------------------------------------------------
# AES-128 UDP ENCRYPTION HELPERS
# Packet format: IV(16 bytes) + ciphertext
#------------------------------------------------------------

def get_aes_key():
    # AES-128 needs a 16 byte key. This turns "teamsocket" into a stable 16 byte key.
    return hashlib.sha256(AES_PASSPHRASE).digest()[:16]


def aes_pad(data):
    # PKCS#7 style padding so the message length becomes a multiple of 16 bytes.
...
def encrypt_udp_packet(plain_data):
    if not UDP_ENCRYPTION_ENABLED:
        return plain_data
    ...
    ciphertext = encryptor.update(aes_pad(plain_data)) + encryptor.finalize()

    # Send the IV first because the receiver needs it to decrypt this packet.
    return iv + ciphertext


def decrypt_udp_packet(packet_data):
    if not UDP_ENCRYPTION_ENABLED:
        return packet_data

    if len(packet_data) < AES_BLOCK_SIZE * 2:
        raise ValueError("encrypted UDP packet is too short")

    iv = packet_data[:AES_BLOCK_SIZE]
    ciphertext = packet_data[AES_BLOCK_SIZE:]

    if len(ciphertext) % AES_BLOCK_SIZE != 0:
        raise ValueError("encrypted UDP packet has invalid length")
    ...
    return aes_unpad(padded_plaintext)
...
def send_udp_message(message, ip=BROADCAST_IP, port=UDP_PORT):
    ...
    plain_data = json.dumps(message).encode("utf-8")
    data = encrypt_udp_packet(plain_data)
...
def udp_listener_thread():
    ...
    while running:
        try:
            encrypted_data, address = udp_socket.recvfrom(4096)

            try:
                data = decrypt_udp_packet(encrypted_data)
            except Exception as decrypt_error:
                print("could not decrypt UDP packet from", address[0], ":", decrypt_error)
                continue
