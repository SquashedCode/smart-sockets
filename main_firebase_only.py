...
USER_NAME = "User"
HUB_REGISTRATION_KEY = "_Hub_Registration"
...
def ensure_firebase_hub_branch():
    ...
    try:
        existing_hub_data = hub_ref.get()

        if existing_hub_data is not None:
            print(f"Firebase hub branch already exists: {hub_ref_path}")
            return

        # Firebase Realtime Database does not keep empty branches.
        # This small rules-compatible marker lets the app see the Hub_1 branch
        # before any bases have been discovered. It is ignored by the display.
        registration_data = make_firebase_safe_device_data({
            "name": HUB_REGISTRATION_KEY,
            "ip": get_local_ip(),
            "Status_base": "Hub_Registered",
            "Node_L": {
                "Attached": False,
                "Power": False
            },
        ...
        hub_ref.child(HUB_REGISTRATION_KEY).set(registration_data)
        print(f"Firebase hub branch created: {hub_ref_path}")

    except Exception as error:
        print("Could not check/create Firebase hub branch:", error)
...
def get_device_from_firebase(device_name):
    if firebase_admin is None or not firebase_admin._apps:
        return None
    ...
    try:
        ...
        print(f"Could not read {device_name} from Firebase:", error)
        return None


def build_device_data(device_name, device_ip, message, existing_data=None):
    if existing_data is None:
        existing_data = {}

    node_l = message.get("Node_L", existing_data.get("Node_L", {
        "Attached": False,
        "Power": False
    }))

    node_r = message.get("Node_R", existing_data.get("Node_R", {
        "Attached": False,
        "Power": False
    }))

    status = (
        message.get("Status_base")
        or message.get("status")
        or existing_data.get("Status_base")
        or "Online"
    )
...
def update_device_from_packet(device_name, device_ip, message):
    existing_data = get_device_from_firebase(device_name)
    device_data = build_device_data(device_name, device_ip, message, existing_data)

    if existing_data:
        old_ip = existing_data.get("ip")

        if old_ip != device_ip:
            print(f"{device_name} IP updated: {old_ip} -> {device_ip}")
        else:
            print(f"{device_name} already exists in Firebase")
    else:
...
def load_firebase_devices_into_memory():
    global devices
    ...
    if firebase_admin is None or not firebase_admin._apps:
        print("Firebase unavailable. Device display will update from live UDP packets only.")
        needs_display_update = True
        return
    ...
    try:
        ...
        print("Could not load devices from Firebase:", error)
        needs_display_update = True
        return

    if not firebase_devices:
        print("No devices found in Firebase for this hub yet.")
        needs_display_update = True
        return

    with devices_lock:
        for device_name, device_data in firebase_devices.items():
            # Ignore internal Firebase marker entries.
            if str(device_name).startswith("_"):
                continue
    ...
    print(f"Loaded {len(devices)} device(s) from Firebase")
    needs_display_update = True
...
def handle_udp_message(data, address):
    ...
    if is_discovery_response(message):
        ...
        device_data = update_device_from_packet(name, ip, message)
        ...
        print(f"Device updated: {name} at {ip}, status={device_data.get('Status_base', 'Online')}")
        needs_display_update = True
